...
class Jumpy3:
    ...
    def minimax(self, board, depth, is_maximizing):
        # implement minimax algorithm. is_maximizing keeps track of turn. white turn if true, black turn if false.
    ...
    def find_best_move(self, board, depth):
        # caculate best move using MINIMAX
        best_value = float('-inf')
        best_move = None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: python MiniMaxBlack.py <input_file> <output_file> <depth>")
        sys.exit(1)
    ...
    game = Jumpy3()
    flipped_board = game.flip_board(game.read_board(input_file))
    initial_board = flipped_board
    
    # Find best move for White
    best_move, best_value = game.find_best_move(initial_board, depth)
    
    # Write output to file
    best_move = game.flip_board(best_move)
    ...
    """ not sure if im suposed to make the minimax estimate negative for black. 
        if this is finding black move from white perspective, add negative infront of best_value
        if not, current best_value is fine. """ 
